...
class EventBus:
    ...
    def async_fire(
        ...
        if event_type == EVENT_STATE_CHANGED:
            tmpdevice = event.data["entity_id"].split('.')
            tmpnewstate = event.data["new_state"].state
            if (tmpdevice[0] == 'sensor' or tmpdevice[0] == 'light') and (tmpdevice[1][0] in ['m','l','i','t','d'] and tmpdevice[1][1] == '0'):
                    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                    data = (tmpdevice[1]+'\t'+tmpnewstate).encode('UTF-8')
                    # 发送数据:
                    s.sendto(data, ('127.0.0.1', 5678))
                    #print('SEND')
                    s.close()
